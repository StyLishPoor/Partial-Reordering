...
int main(int argc, char* argv[]) {
  if ( argc < 8 ) {
    std::cout << "usage : ./ex [input_graph] [return_prob] [RW_NUM] [minimun_node_num] [original_output_txt] [sequential_output_txt] [random_output_txt]" << endl;
    return 0;
  }
  ...
  Mapping Sequential_Mapping, Random_Mapping;
  Group DDBG_Group(8);
  // debug 用のカウンタ
  ...
  } while (Partial_Graph.size() < mini_node_num);
  ...
  random_shuffle(visited_order.begin(), visited_order.end());
  ...
  ofstream ofs_sequential(argv[6], ios::trunc); // 完全なDBG 
  ofstream ofs_random(argv[7], ios::trunc); // 完全なDBG 
  //string line;
  //string source;
  //string target;
